...
- **Capability evaluation results (MMLU + GSM8K, 4-way).** `outputs/tables/capability_eval.csv` now contains the full base / SFT / PPO / DPO matrix on a 50-question slice of each benchmark. Headline finding: the DPO-vs-SFT/PPO alignment-tax advantage is roughly **7× on both benchmarks** — MMLU 0.2pp vs 1.5pp, GSM8K 2pp vs 14pp. PPO and SFT are statistically identical on both, consistent with PPO's declining-reward training trajectory.
